            ...
            <div className="space-y-12">
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-gray-900">ElevenLabs Voice</h3>
                    <p className="text-xs text-gray-400 font-bold">Real human-quality voice · Each user uses their own free key</p>
                  </div>
                  <a href="https://elevenlabs.io/app/settings/api-keys" target="_blank" rel="noreferrer"
                    className="ml-auto px-4 py-2 bg-violet-500 hover:bg-violet-600 text-white font-black text-xs rounded-xl transition-all whitespace-nowrap">
                    Get Free Key →
                  </a>
                </div>
                <div className="bg-violet-50 border border-violet-100 p-4 rounded-2xl">
                  <p className="text-xs font-bold text-violet-800">
                    ElevenLabs gives <strong>10,000 free characters/month</strong>. Get your own key at elevenlabs.io — it's free.
                    Voice starts playing in <strong>1-2 seconds</strong> with streaming. Much faster than before.
                  </p>
                </div>
                <div className="relative">
                  <input
                    type="password"
                    placeholder="sk_... (your ElevenLabs API key)"
                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 font-bold outline-none focus:border-violet-500 transition-all"
                ...
                {/* Voice picker */}
                <div>
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Choose Voice</p>
                  <div className="grid grid-cols-1 gap-2">
                    {[
                      ...
                      { id: "custom", name: "Custom Voice ID", desc: "Paste your own ElevenLabs Voice ID" },
                    ].map(v => {
                      const currentId = (data.settings as any).elevenLabsVoiceId || "UmQN7jS1Ee8B1czsUtQh";
                      const isActive = v.id === "custom"
                        ? !["UmQN7jS1Ee8B1czsUtQh","19STyYD15bswVz51nqLf","nDJIICjR9zfJExIFeSCN"].includes(currentId)
                        : currentId === v.id;
                      return (
                        <button key={v.id} onClick={() => {
                          if (v.id !== "custom") updateData({ settings: { ...data.settings, elevenLabsVoiceId: v.id } as any });
                        }}
                          className={`flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all ${isActive ? "border-violet-500 bg-violet-50" : "border-gray-100 hover:border-gray-300"}`}>
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg ${isActive ? "bg-violet-500 text-white" : "bg-gray-100 text-gray-400"}`}>
                            {v.name.charAt(0)}
                          </div>
                          <div className="flex-1">
                            <div className="font-black text-sm text-gray-900">{v.name}</div>
                            <div className="text-[10px] font-bold text-gray-400">{v.desc}</div>
                          </div>
                          {isActive && <span className="text-violet-500 font-black text-xs">✓ Selected</span>}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="mt-3">
                    <input
                      type="text"
                      placeholder="Paste custom Voice ID from elevenlabs.io/app/voices"
                      className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3 text-sm font-bold outline-none focus:border-violet-500 transition-all"
                      value={["UmQN7jS1Ee8B1czsUtQh","19STyYD15bswVz51nqLf","nDJIICjR9zfJExIFeSCN"].includes((data.settings as any).elevenLabsVoiceId || "UmQN7jS1Ee8B1czsUtQh") ? "" : ((data.settings as any).elevenLabsVoiceId || "")}
                      onChange={(e) => { if (e.target.value) updateData({ settings: { ...data.settings, elevenLabsVoiceId: e.target.value } as any }); }}
                    />
                  </div>
                </div>
                {(data.settings as any).elevenLabsKey && (
                  <div className="flex items-center gap-2 text-green-600 text-xs font-black">
                    <span className="w-5 h-5 bg-green-100 rounded-full flex items-center justify-center">✓</span>
                    ElevenLabs connected — voice will stream instantly
                  </div>
                )}
              </div>
            </div>
